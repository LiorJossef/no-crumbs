...
describe('what the entrance may not delay', () => {
  ...
  /** Every scope that is not playing an entrance renders exactly as it did — the collections views
   *  are search params on this same mount and say nothing about the entrance either way. */
  it('is off by default for every other scope', () => {
    ...
    expect(
      readFileSync('src/app/map/collections-scope.tsx', 'utf8'),
