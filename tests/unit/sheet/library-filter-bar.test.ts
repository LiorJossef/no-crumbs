...
describe("LibraryFilterBar — one trigger per axis, because they are not one question", () => {
  ...
  it("leaves out an axis that cannot change the result", () => {
    ...
    const chips = buttons(
      render({
        facets: [{ category: "restaurant", count: 12 }],
        anyVisited: false,
        tagFacets: [],
      }),
    );
...
describe("LibraryFilterBar — every trigger states its own value without being pressed", () => {
  ...
  it("names one selected tag and counts several, because three names would be the whole row", () => {
    expect(buttons(render({ activeTags: ["wine"] }))[2]).toContain(
      'aria-label="Tags, Wine"',
    );
    const many =
      buttons(render({ activeTags: ["wine", "late night"] }))[2] ?? "";
  ...
  it("is quiet at rest: no fill, no bold, and mint only once that axis is on", () => {
    ...
    expect(buttons(render({ visitFilter: "been" }))[0]).toContain(
      "bg-tag-selected",
    );
  });
  ...
  it("does not let a press on a trigger start a sheet drag", () => {
    expect(
      (render().match(/data-vaul-no-drag/g) ?? []).length,
...
describe("LibraryFilterBar — Clear is a word, and only when there is something to clear", () => {
  ...
  it("says what it does, in words, at the touch floor", () => {
    ...
    expect(markup).toMatch(
      /<button[^>]*aria-label="Clear all filters"[^>]*class="[^"]*min-h-11/,
    );
...
describe("the inline panel is measured against the sheet, not the viewport", () => {
  ...
  // Quote-agnostic on purpose. This assertion read `'--sheet-content-height'` in single quotes
  // while the file it reads is double-quoted, so it matched nothing and the guard was dead for as
  // long as it existed — found 2026-09-03 while building the card's side of the same mechanism.
  const PUBLISHES = String.raw`["']--sheet-content-height["']`;

  it("is published by the same element that sets the column’s height", () => {
    expect(SHEET).toMatch(
      new RegExp(
        String.raw`height: STOP_TO_CONTENT_HEIGHT\[stop\],[\s\S]{0,600}${PUBLISHES}: STOP_TO_CONTENT_HEIGHT\[stop\]`,
      ),
    );
  });
