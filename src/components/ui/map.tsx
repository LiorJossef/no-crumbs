...
/* eslint-disable react-hooks/refs, react-hooks/set-state-in-effect -- vendor source, installed
 * verbatim from the mapcn shadcn registry (https://www.mapcn.dev/r/map.json). It wraps an
 * imperative MapLibre `Marker`/`Popup` instance per component and wires that instance's own event
 * listeners onto a ref-held latest callback, and stages a MapLibre style swap across two effects
 * (close the gate, then swap) — both standard imperative-escape-hatch patterns for driving a
 * non-React canvas library, not accidental render-time reads, but ones this project's stricter
 * react-hooks lint rules (added since this repo's eslint config was written, ahead of this
 * install) flag anyway. Disabled file-wide rather than patched line-by-line so future `shadcn
 * add` re-installs of this component stay a clean diff against upstream. */
