...
| 2026-08-22 | **The 2026-08-22 Google-coordinates flag is superseded, not closed.** A first ruling rejected it (bright-line ToS conflict, Service Specific Terms §3.3/§5.3: Google content may not pair with a non-Google map, on top of the 30-day cache limit) — but that ruling assumed the renderer stays MapLibre+CARTO and storage must be forever. The owner corrected both assumptions the same session: the map itself may move to Google Maps (reopening D2's §2 half, not just §3), and periodic refresh in place of permanent storage is acceptable. Under the corrected premise the ToS blocker likely does not apply — Google content on a Google map with a refresh cycle is within their terms. `06-map-and-places-decision.md` §3.3 is a placeholder pending re-evaluation; the stale rejection was not left standing as current. Re-ruling owed next, scoped correctly this time as a possible **D2 map-provider reopen**, not a narrow coordinate-source add-on |
