...
| 2026-08-30 | **NAV-2 landed; the camera deferred to its own pass before submission (owner ruling).** PR #104 squashed onto `main` as `250f524` and deployed: `/collections` is now the shell's sheet over the user's own map rather than a standalone document (no `<h1>`, no `<header>`, no `max-w-[560px]` column, and neither of the two controls that pointed at a map the user was already inside), plus a peek line for both collection scopes and one focus move on entering a collection. `one-shell.test.ts` guards the deletions — one `Drawer.Root`, one `MapSurface` call site, one declaration of the sheet geometry across `src/`. **Deferred, recorded rather than done, and scheduled by the owner for the run-up to submission:** the camera is to be handled as one focused piece of work rather than symptom-patched inside a navigation refactor. Two things ship broken and are known: (1) `/collections/[id]` at desktop width frames 15 central-London pins across ~200 km of southern England — the centre is right, the scale is several zoom levels wrong, and every pin lands in one untappable clump; the sibling index clips its pins at the fit edges. Pre-existing vs introduced by `4f5b254` is **unmeasured**. (2) All five of the camera movers left undriven by `4f5b254`'s collapse of `focusPlaceIds` + `focusBounds` into a single focus slot — country pill, area pill, mover 5, post-import, near-me — are **UNVERIFIED**, neither shown to pass nor to fail; a verification run was stopped before it reported. Draft (unrun, uncommitted) specs for four of them sit untracked at `tests/e2e/camera-movers.spec.ts`, `tests/e2e/camera-post-import.spec.ts` and `tests/e2e/helpers/map-camera.ts`. Full detail in `docs/handoff-camera-framing.md`. Merged by the owner with `--admin`: CI is configured but failing on exhausted Actions minutes (all four jobs dead in 2–3 s, no logs), so `merge:pr` refused and local evidence stood in — clean `next build`, 2017 unit tests, clean typecheck, and a new collections e2e spec green at both breakpoints |
