...
### L0-F2 — Local resolve seam · `maps-geospatial` · depends: — · cut: never · **RESUMED 2026-08-27**
...
### L0-F3 — Global resolver (D2b) · `maps-geospatial` + `security-privacy` · depends: F2 · cut: never · **RESUMED 2026-08-27**
The feature that makes the MVP global. Carries the only gate outside its owner's control.
**Paused 2026-08-22; resumed 2026-08-27** under `06` §3.4's own exit clause ("or sooner, if
AI-based resolution proves too inaccurate to be useful") — the measurement is 65–470 m of coordinate
error. `T1`, the ODbL sign-off, is a **merge gate** and starts first: it is a document, not code.
...
**T1's block is resolved, 2026-08-27: it waits behind the resolver, which is now being built.**
"An un-ingested city" is the `PlaceResolver` of `L0-F2b`/D2b. That was a standing block only while
the resolver was paused indefinitely; `L0-F2`/`L0-F3` resumed on 2026-08-27 under `06` §3.4's own
exit clause, so T1 now has a real dependency with a real owner rather than an open question. It is
**not** the next branch: manual add was reconsidered and rejected as the next workstream because it
adds a second path to create places while the main path still puts pins 65–470 m from the venue —
see this file's change log for the full reasoning.
