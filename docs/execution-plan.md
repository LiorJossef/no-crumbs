...
2026-09-01. Those live in [`archive/feedback-round-3-work-plan.md`](archive/feedback-round-3-work-plan.md), and the
running state is [`current-state.md`](current-state.md) with
[`archive/handoff-2026-09-02-ui-density.md`](archive/handoff-2026-09-02-ui-density.md) beside it.
...
| **The filter row is parked, not abandoned** | Ten owner rulings for it are in `archive/feedback-round-3-work-plan.md` §5.1. It churned through ten corrections in one session because **this product has no menu or dropdown specification** — `docs/ux-menus-and-dropdowns.md` should be written before it resumes |
...
| **Not in this ladder at all, and the highest risk on the board** | The **10–15 minute presentation deck** (M11, M12 item 10). Submission is **6 September 2026**. `archive/presentation-outline.md` is an outline, not a deck. Every other graded artefact exists |
...
**Ranked second of the three after-the-save capabilities** (`docs/archive/product-ruling-after-the-save.md`
...
**New, `product-lead` ruling 2026-08-29 — `docs/archive/product-ruling-after-the-save.md`.** Numbered F12 so
...
**Blocked on an owner ruling (OD-1 in `docs/archive/product-ruling-after-the-save.md` §5), and deliberately
...
### L2-F1 — Collections and shared collections · `supabase-database` + `design-system-frontend` + `security-privacy` · depends: L1-F5, L1-F6 · **BUILT 2026-08-30, ahead of L1 closing**

Built out of order on the owner's explicit overnight instruction, which supersedes both the L2
deferral in `mvp-plan.md` and the backlog's own §9 recommendation to stop at user labels. The
reasoning in those two entries was not overturned — the *sequencing* was. `user_tags` and OD-1 are
...
| T6 — ownership transfer | **NOT BUILT, and it blocks account deletion** | `collections.owner_id` is `on delete cascade`, so an owner deleting their account destroys the collection for everyone in it, and an owner cannot leave their own collection. Must land before backlog §11.8 |

**The decision the whole feature rests on:** a collection item points at a `places` row, never at a
`saved_places` row. The 2026-08-28 ruling told us to expect membership-based RLS on every
`saved_places` path; pointing at the shared identity row instead meant sharing opened **exactly one**
new policy and the hot read was never touched. The consequence is a privacy property rather than an
implementation detail — the adder's note, tags, source link and above all `visit_state` cannot reach
a collaborator, because no policy grants them.

Design: `docs/archive/ux-collections.md`. Session account: `docs/archive/handoff-2026-08-30-collections.md`.
...
`docs/archive/product-ruling-after-the-save.md` §4: the trip case ("what did I save for Tokyo?") is answered
...
shopfronts as venues (`archive/handoff-2026-08-28-google-places-primary.md`). It should stop appearing on
...
### L2-F2 — Natural language search · `ai-extraction` + `design-system-frontend` · depends: L1-F6, L1-F12 · cut: 3 · **PLANNED 2026-09-04, NOT STAFFED**

Full plan: [`docs/nls-plan.md`](nls-plan.md). The user types a sentence, a small model turns it into
a structured intent, and **the deterministic filter chain that already ships does the filtering** —
the model never returns places. Zero migrations across both stages.

**V1 is a separate search surface** (owner ruling, 2026-09-04): its own panel with an explicit
submit, the existing search field untouched. Unification with the live literal field is deferred
until this has been used against a real library; `nls-plan.md` §7 records the four candidate designs
and why two of them are already rejected, so that argument is not had twice.

| Task | What | Owner | Exit criterion | Status |
|---|---|---|---|---|
| T1 | Run the existing 35-case golden set live against `gemma-4-26b-a4b-it` | `ai-extraction` | Scored, published in `docs/evidence/`, **≥ 90% no-false-filter** and ≥ 70% exact intent, he/en reported separately | todo |
| T2 | `domain/search/intent.ts` — schema, validation, the library clamp | `ai-extraction` | A hand-injected intent naming a value the library lacks produces no chip | todo |
| T3 | `api/search/interpret` route + `integrations/llm/query-intent.ts` | `ai-extraction` | One call per submit, hard timeout, no retry, failure leaves the panel editable | todo |
| T4 | The sentence panel, both breakpoints | `design-system-frontend` | Preview count always equals the places shown after apply; existing search verified unchanged **by use** | todo |
| T5 | Geography: country → locality → **expand to cluster** | `maps-geospatial` | All four stored Tel Aviv spellings return the same 20 places | todo |
| T6 | Camera + undo as one transaction | `design-system-frontend` | Flies once through mover 4/5, reduced-motion respected, undo returns it, **mover docblock amended in the same commit** | todo |

**The measured defect T5 exists to close:** the library holds four spellings of Tel Aviv, and
`clusterByProximity` has *already* merged them into one 20-place area. Today `tel aviv` returns 7
rows and `תל אביב` returns 13. Neither returns 20.

**Two axes in the original brief are not axes.** `sources.platform` is `CHECK IN ('tiktok')`, so
source cannot discriminate — `saved_places.origin` is the real one. Collections are a separate
filter model with no tag/visit/category axes; folding them in means unifying two systems, and they
are deferred.

**Blocked on an owner decision before T5 ships:** Stage 2's prompt sends the user's own city names
to the provider. That is location data leaving the device and needs `security-privacy`. T1–T4 are
unaffected — Stage 1's prompt carries no city names.

**Semantic dish/note search is out of scope** and parked in `nls-plan.md` §8. Half of it already
works (`search.ts` searches `dishes` today); the remaining gap is cross-language translation, not
retrieval, and the answer is query expansion rather than pgvector.
...
| 2026-09-04 | **Natural language search planned (`L2-F2`), not built** — [`docs/nls-plan.md`](nls-plan.md). Five specialists measured the code and the local database before any design: the filter chain is four pure passes over four `useState` cells in one component, so an intent layer writes state and needs no new execution path; **`clusterByProximity` has already merged all four stored Tel Aviv spellings into one 20-place area**, which turns geography from a gazetteer problem into "match any spelling, expand to the cluster"; the dish corpus is ~50 of 58 strings in Hebrew, so `tsvector` is worse than useless and the Stage 3 gap is translation rather than retrieval; and **`docs/evidence/.local/nls-benchmark-dryrun.json` already holds a 35-case he/en golden set aimed at `gemma-4-26b-a4b-it`, marked `"DRY RUN — no call was made"`** — the acceptance gate was built on 2026-08-30 and never run. Gemma 4 is real (2026-03-31, Apache 2.0, free tier on the Gemini API) and this repo has already verified `responseSchema` works with it, but it was **rejected once for extraction** over longitudes wrong by 15–40°; that is world knowledge, which intent parsing does not need, so it ships gated on the golden set with `gemini-3.5-flash-lite` one env var away. **Three UX revisions in one session, all before code**: Enter-in-the-existing-field was proposed and rejected by the owner (it gives one control two rules, and lies on mobile), an automatic offer bar replaced it, and the owner then ruled **V1 ships as its own surface with the existing search untouched** — which deleted the entire "when do we call?" subsystem (debounce, gate, cache, call cap) and left exactly one call per submit. Zero migrations |
