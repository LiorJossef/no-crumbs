...
| R2 | A bottom bar with paged destinations — Map · Collections · Profile · create. Reverses "no tab bar" | `archive/ux-navigation-structure-2026-08-29.md` banner |
| R3 | No bar that floats *over* the sheet: dead scroll zone at `half`, gesture conflict in the flick band | `archive/ux-navigation-supplement-2026-08-29.md` §1 |
| R4 | The `＋` is one global create menu, present on every tab | `archive/ux-import-flatten.md` §7 |
| R5 | The unit of list scope is a place **cluster**, never a rectangle | `ux-stable-area-list.md` |
| R6 | The list settles, it never tracks: recompute at `moveend` + 120 ms, nothing moves mid-gesture | `ux-stable-area-list.md` (the rule survived; `archive/ux-map-is-the-query.md` §4's viewport-following did not) |
| R7 | `3 of 20` is retired; the library total appears nowhere on `/map` | `archive/ux-map-is-the-query.md` §2.2 |
| R8 | Area label needs ≥70% of one normalised locality, else `this area` — decline to name rather than pick a spelling | `archive/ux-map-is-the-query.md` §2.1 |
| R9 | Never build the `Search this area` pill | `archive/ux-map-is-the-query.md` §8 |
...
| R16 | One confident result collapses: no tickbox, no card chrome, the name as H1 | `archive/ux-import-flatten.md` §3 |
| R17 | The confirm step itself is never removed (Charter §3 invariant 2) | `archive/ux-import-flatten.md` §7 |
...
| R25 | Distance is displayed only against a real fix — never distance from a map centre | `archive/ux-map-is-the-query.md` §6 |
| R26 | The list is the accessible representation of the map; pins are canvas and have no nodes | `archive/ux-map-is-the-query.md` §7.2 |
| R27 | Live regions are search-driven; panning announces nothing | `archive/ux-map-is-the-query.md` §7.1 |
...
| R32 | No second overlay: every view swaps the content of the surface it is already in | `archive/ux-navigation-supplement-2026-08-29.md` §3 |
...
| R35 | Categories are a filter bar, not a destination | `archive/ux-navigation-structure-2026-08-29.md` §3 |
| R36 | No `All` chip — nothing pressed *is* everything | `archive/ux-category-filter-notes-2026-08-29.md` §5 |
| R37 | Facet counts are live and scoped to the other filters, not to the active area | `archive/ux-category-filter-notes-2026-08-29.md` §2.5, §5 |
| R38 | Phone and desktop render the same filter bar | `archive/ux-category-filter-notes-2026-08-29.md` §5 |
| R39 | An empty **collection** offers your recent places; an empty **library** offers nothing — suggesting places is recommendation | `archive/ux-navigation-supplement-2026-08-29.md` §2 |
| R40 | The empty library opens on a regional map from the browser timezone: no IP lookup, no permission prompt | `archive/ux-map-is-the-query.md` §5 |
| R41 | No illustration, mascot, carousel, checklist, gradient or `0 places` on any empty state | `archive/ux-map-is-the-query.md` §5, `spec-no-places-found.md` §4.4 |
| R42 | Never convert uncertainty into certainty: an approximate pin says so, in the same place the user reads the name | `archive/ux-import-flatten.md` §4 |
...
| R46 | The shell unification is **not** deferred behind the chrome work: the duplicate drawer and second map surface are what make the route feel like a separate app, which is the complaint itself (owner, 2026-08-30, closing `O13`) | `archive/product-ruling-one-place-one-object.md` R4 |
...
| O2 | `NAV-ALT` — the only honest bar: resting chrome, peek stop deleted, `PEEK_PX` moved in four places. Multi-day, owner's call | `archive/ux-navigation-supplement-2026-08-29.md` §1.5 |
...
| O4 | Does `Add a place` deserve its own tap, or does `＋` open the field directly? | `archive/ux-import-flatten.md` §8.1 |
| O5 | Widen the `preselect` band? 3 of 16 ask a question with 0 genuine ambiguities. Needs a measurement pass first | `archive/ux-import-flatten.md` §8.2 |
...
| O7 | `Show my places` vs the literal `Show all places`. Behaviour is settled either way | `archive/ux-map-is-the-query.md` §10 |
