...
| Component stack | **shadcn/ui + Tailwind CSS + Lucide icons + Manrope** (the "neutral variable sans" above), decided 2026-08-20 — before the name and the token values, because shadcn's copy-in-source components are the *reason* this is safe to fix early: restyling later is a token/class change, not a rewrite. **No UI primitive is hand-rolled** while a shadcn equivalent exists. **Motion** (the successor to Framer Motion) is the animation library, used only for subtle interaction/state-transition motion — not for the five choreographed motion moments, which stay `ux-architecture`'s call and L2-scoped |
