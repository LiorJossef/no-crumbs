...
| # | Defect | Evidence |
|---|---|---|
| **G1** | **The zero-state screen was designed, specified, and never built.** `showImport` is `useState(false)`; `archive/ux-map-is-the-query.md` §5 specifies the empty-library screen — regional map from the browser timezone, import overlay auto-opened — with **"no new components and no permission prompt"**, and line 6 of that same document admits the overlay is unbuilt. The first screen a new user sees is one sentence and a plus button | `map-page-client.tsx:170` |
