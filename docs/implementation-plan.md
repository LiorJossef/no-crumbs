...
3. The per-case scores are golden-filed against `raw-overture-scored.json`, not just the band labels,
   ...
   **Met for `name_score`, `token_cov` and `cat_match` on 2026-08-19 (task 4): 220/220 rows.
   ...
   `poi_index.dataset_confidence`, not here, and the prefix-scale risk this criterion was written
...
| 5 | **Ingest Tel Aviv only**, against a pinned Overture release, with the region scoping from `06` §6.1 step 2. Record the row counts. **Rider from task 4:** also emit the per-`overture_id` `confidence` the benchmark never recorded, as a `measure-dataset-confidence.py` sibling under `docs/evidence/places/`. Without it the `score` column of exit criterion 3 stays unprovable — `conf = 0.5` reproduces 0 of 220 rows | `maps-geospatial` | One city is enough to prove the pipeline end to end. Tokyo and London are task 6, and are the first thing to cut |
