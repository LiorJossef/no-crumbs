...
| 2026-08-23 | **Free-web-search extraction provider explored, and deferred.** Looking for a genuinely-free, provider-agnostic `PlaceExtractor` with web search (via the Vercel AI SDK) surfaced three dead ends, each verified live rather than assumed: OpenRouter's free `google/gemma-4-26b-a4b-it:free` returns markdown-fenced JSON (fixed via OpenRouter's `response-healing` plugin) but sits on a shared free-tier pool that rate-limited repeatedly in testing; `gemini-2.5-flash` — the only Gemini tier with a documented free Search-grounding quota (500 RPD, no billing) — 404s for this project's API key ("no longer available to new users"); every reachable Gemini 3.x model (`gemini-3.1-flash-lite`, `gemini-3.5-flash-lite`, `gemini-3.6-flash`) 429s on grounding with `RESOURCE_EXHAUSTED`, confirming 3.x grounding needs a paid account regardless of Flash/Lite variant. D7's default (Anthropic `claude-haiku-4-5`) is unchanged. Groq's `compound-mini` (free tier, 250 RPD, built-in Tavily-backed web search, first-party `@ai-sdk/groq`) was identified as the one live-unverified-but-documented option not yet dead-ended, alongside a separate open question — whether a search-grounded model should return coordinates directly rather than only a name for the existing resolver — that touches D2b and belongs to `maps-geospatial`, not this thread. Both stashed rather than pursued: `git stash` entry "gemini grounding attempt — blocked on 429 quota, deferred" on branch `feat/gemini-flash-default-websearch` (one live commit, `1b4c401`, swaps the adapter's model id to the now-404ing `gemini-2.5-flash` — needs correcting before that branch is resumed) |
