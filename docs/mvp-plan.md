...
| Migrations `0010` + `0014` (POI index, `pg_trgm`, provenance columns) | **applied** (updated 2026-08-24 — L0 step 4 closed) | L0 step 4 |
| Migrations `0011`–`0016` (auth/session support, source-link + thumbnail denormalization on `saved_places`) | **done**, added 2026-08-24 — not part of the original nine, feeds L1 UI ahead of strict order | L1 |
...
| The global resolution path — provider chosen (`06` §0), provider/dataset enums declared in `domain/types.ts`, `places` provenance columns and `resolve_place()` writes, `cityHint`/`countryHint` in the extraction contract | **designed, not built, and deferred by owner instruction** (`execution-plan.md` 2026-08-22/2026-08-24 entries) — no Nominatim adapter exists; the interim path is an LLM-guessed venue plus a Google Maps search link for human verification | L0 step 3 |
...
**→ L2, and the order matters — near-me is now first:** near-me and geolocation, **promoted to the
first L2 item** because it serves the everyday half of the single primary user in
[`brand-and-product-foundation.md`](brand-and-product-foundation.md) §2 · the forked Protomaps style and the premium bar · clustering ·
