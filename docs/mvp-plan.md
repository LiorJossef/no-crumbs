...
| 10 | User location and "what did I save near me" | **yes**, owner ruling 2026-08-27 (was → L2) |
...
| — | **Been / not been yet** — the library's completion state | **yes, added 2026-08-29** (`L1-F12`). Not one of Charter §4's fourteen, and it is not a scope addition either: `saved_places.visit_state` / `visited_at` have existed since migration `0006`, inside the user's own UPDATE column grant, and are read by no screen. See below |
...
UPDATE grant to `authenticated`, all shipped in `0006` and wired to nothing. It is a **per-user
