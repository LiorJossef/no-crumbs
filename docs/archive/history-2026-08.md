...
> was tried, what was measured, what the owner ruled, and the reasoning behind decisions we should
> not reopen. Several of its claims were already false when they were written down — production
> being "down", commits being "unpushed", features listed as unbuilt that had shipped. They are left
> exactly as they were, because the point of a history is what was believed at the time.
>
> The per-session detail also lives in the `handoff-*.md` files.
