...
| Item | Why |
|---|---|
| **§1.1** mixed he/en baseline **on the map** | **PARTLY WRONG, corrected 2026-09-02 — and it is why a real defect sat deferred.** The reasoning below rested on *"the RTL audit already confirms mixed-run ordering is correct"*, and concluded that only baselines remained, which looked structural and expensive. False for the **summary pills**: the count sitting off-place beside a Hebrew city name is an *ordering* defect, not a metrics one, and the owner raised it directly. Fixed in `62555e3` by drawing the pill as one bitmap — bidi controls do nothing here, because `mapbox-gl-rtl-text` resolves direction from the label's own script and ignores `U+2068`/`U+2069` and `U+200E`. **What remains genuinely deferred** is only the baseline half: Hebrew glyphs come from CARTO's Positron PBFs in a Latin-metric font stack, so mixed runs on the *basemap* get different baselines. That needs a glyph source or an explicit stack — M–L, and cosmetic |
