...
| `tiktok-recognition.md` / `tiktok-recognition-run.json` | **Added 2026-08-27 (TLV-CORPUS-1, `qa-reliability`).** The same resolver measured on **real TikTok captions** instead of hand-written queries: [`tests/manual/tiktok-recognition.manual.ts`](../../../tests/manual/tiktok-recognition.manual.ts) takes the URLs in [`tests/manual/tiktok-recognition-corpus.json`](../../../tests/manual/tiktok-recognition-corpus.json) through the shipped oEmbed → caption → extraction → resolve path and reports the **auto-match rate** — the share of extracted candidates that land in `preselect` **and** are the right venue. It exists because every number in `tlv-resolve-benchmark.md` comes from queries that are script-matched to the index by construction (a Latin query for a Latin-named row), which is exactly the case real captions are not: 64% of the index is Hebrew-named with no aliases. Misses are bucketed by cause — absent from index / present but unreachable across scripts / ranking / region never searched / extraction produced nothing / correct but could not auto-accept — with an independent `poi_index` ILIKE probe separating the first two. A case with no `expected` block is reported `unadjudicated` and counts toward the rate in neither direction. oEmbed responses and extractions are cached to `docs/evidence/.local/` (gitignored), so a re-run costs zero LLM calls. **First real measurement, 2026-08-27: 13 URLs, 16 adjudicated candidates, an auto-match rate of 4/16 (25%) and zero false auto-accepts** — against 8/15 on the synthetic benchmark, which is the gap this file was built to expose. |
