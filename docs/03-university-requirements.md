...
| M9 basic security | **OUTSTANDING — owner deferred.** `docs/security.md` is an INTERIM file holding only the membership-gate ruling plus a 12-item owed list. The practices exist in the design (RLS forced, server-only secrets, Zod at boundaries, no persisted location); the graded document does not. Must be produced before submission | `docs/security.md` §3 |
